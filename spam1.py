...
def run(phone, delay, i):
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                ...
                for future in concurrent.futures.as_completed(futures):
                    try:
                        ...
                        console.print(
                            Panel(error_message, title="[bold red]LỖI[/bold red]", border_style="red")
                        )

            console.print(Align.center(Panel(
                f"[bold cyan]✓ SPAM THÀNH CÔNG LẦN {i}![/bold cyan]",
            ...
            with Progress(transient=True) as progress:
                task = progress.add_task(
                    "[bold magenta]⏳ ĐANG CHỜ...[/bold magenta]",
                    total=delay
                )
...
if __name__ == "__main__":
            ...
            # Kiểm tra tham số
            if len(sys.argv) != 3:
                console.print("[red]❌ Dùng:[/red] python spam.py <sdt> <so_lan>")
                sys.exit(1)
            ...
            try:
                ...
                console.print("[red]❌ Số lần phải là số[/red]")
                sys.exit(1)

            delay = 5  # ⏱️ delay giữa mỗi vòng

            # Chặn số đặc biệt
            if phone in ['0329270154']:
                print("cốn cái lò")
                sys.exit()

            # Thông tin
            console.print(Columns([
                Panel(phone, title="SỐ ĐIỆN THOẠI", border_style="green"),
                Panel(f"{delay}s", title="DELAY", border_style="cyan"),
                Panel(str(count), title="SỐ LẦN", border_style="magenta"),
            ], align="center"))

            console.print(Align.center(Panel(
                "[bold white on red]🚀 BẮT ĐẦU SPAM[/bold white on red]",
                border_style="red"
            )))
            ...
            console.print(Align.center(Panel(
                "[bold white on green]🎉 HOÀN TẤT[/bold white on green]",
                border_style="green"
            )))